...
export interface SummaryDetailItem {
    label: string; // Título (Ej: "Criticidad")
    value: string | number | boolean | React.ReactNode; // Permite texto simple o componentes
    style?: React.CSSProperties; // Opcional para estilo de columna completa, etc.
}
...
export interface DynamicSummaryDisplayProps {
    // Array de secciones formateadas por la aplicación
    sections: SummarySectionData[]; 
    
    confirmationMessage: string; // Mensaje final
}
