        ...
        Object.entries(columnFilters).forEach(([field, filterValue]) => {
            ...
            currentData = currentData.filter(row => {
                ...
                if (
                    ...
                    ('true' in (filterValue as any) || 'false' in (filterValue as any)) &&
                    dataType === 'boolean'
                ) {
                    const sel = filterValue as { 'true'?: boolean; 'false'?: boolean };
                    const selTrue = !!sel['true'];
                    const selFalse = !!sel['false'];
 
                    // Regla del componente: si ambos o ninguno => no filtra (equivale a true global)
                    if ((selTrue && selFalse) || (!selTrue && !selFalse)) return true;
 
                    const b = toBool(rowValue); // normaliza a boolean
                    if (b === null) return false; // valor no interpretable como boolean
