...
const showNotification = (title: string, text: string, icon: 'success' | 'error' | 'warning') => {
    ...
    let config: any = {
        ...
        title: title,
        text: text,
        showConfirmButton: false, // Generalmente se oculta el botón en notificaciones toast/rápidas
        timer: 2000, 
        timerProgressBar: true,
        // Configuración base para el éxito/error
    };

    if (icon === 'success') {
        config = {
            ...config,
            title: `¡${title}!`, // Añadimos exclamaciones
            html: `<div style="font-size: 1.1em; color: #724cafff;">🎉 ${text}</div>`, // Emojis y estilo en el detalle
            showConfirmButton: true, // Mostramos el botón para que el usuario presione OK
            confirmButtonText: '¡Entendido!',
            confirmButtonColor: '#806EAF', // Usamos tu primary-color
            timer: undefined, // Desactivamos el timer si hay botón de confirmación
        };
    } else if (icon === 'error') {
        config = {
            ...config,
            title: `¡ERROR CRÍTICO!`,
            text: `No hemos podido completar la acción. Detalles: ${text}`,
            showConfirmButton: true, 
            confirmButtonText: 'Revisar',
            confirmButtonColor: '#E24A4A', // Usamos tu error-color
            timer: undefined,
        };
    }
...
            await onSave(employ, patchPayload as Partial<PersonModel> & EmployFormData);
            showNotification(
                isCreation ? "¡Creación Exitosa!" : "¡Actualización Exitosa!", 
                `El empleado ${formData.givenName} ${formData.surName} se guardó correctamente.`, 
                'success'
            );
